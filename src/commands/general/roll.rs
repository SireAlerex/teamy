...
impl Roll {
    pub fn builder(number: i64, size: i64, modifier: i64, dk: DropKeep, dk_val: Option<i64>) -> Roll {
        Roll { number, size, modifier, dk, dk_val }
    }
...
fn regex_roll(roll: impl ToString) -> Result<Roll, CommandError> {
    let re = match regex::Regex::new(r"(?<number>\d+)?d(?<size>\d+)(?<modifier>\+\d+|-\d+)?(?<dk>k|d)?(?<dk_val>\d+)?") {
        Ok(r) => r,
        Err(e) => {
            return Err(utils::command_error(format!("erreur regex : {e}")));
        }
    };
    ...
    if !(1..=100).contains(&number) {
        return Err(utils::command_error("le nombre de dés doit appartenir à [1; 200]"));
    }
    ...
    if size <= 1 {
        return Err(utils::command_error("la taille du dé doit être supérieure strictement à 1"));
    }
...
fn rolls_str(rolls: &[i64], modifier: i64) -> String {
    add_modifier(rolls.iter().map(|r| r.to_string()).collect::<Vec<String>>().join(" + "), modifier)
}

fn drop_low(mut rolls: Vec<i64>, n: usize) -> Vec<i64> {
    let bad_elements = rolls.clone().into_iter().sorted().rev().skip(rolls.len() - n).collect::<Vec<i64>>();
...
fn start_message(roll: &Roll) -> String {
    ...
    format!("`[{n}d{size}{show_mod}{dk}{dk_val}]`")
}
...
pub fn run_chat_input(options: &[CommandDataOption]) -> InteractionResponse {
    ...
    let roll = Roll::builder(n, size, modifier, DropKeep::None, None);
